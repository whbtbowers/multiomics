...
for (i in 1:length(datasets)){
  ...
  png(paste("../../img/ex_5/",  date, "/", abbrv ,"_glmnet_alpha", alpha, ".png", sep=""),
  ...
  if (verbose == "gene"){
    ...
    png(paste("../../img/ex_5/",  date, "/sel_", abbrv ,"_glmnet_alpha", alpha, ".png", sep=""),
        width = 5*300,        # 5 x 300 pixels
        height = 5*300,
        res = 300,            # 300 pixels per inch
        pointsize = 8         # smaller font size
    ...
    write.csv(sel.data, file=paste("../../data/ex_4/sel_", abbrv, "_data_body_alpha", alpha, ".csv", sep=""), row.names=TRUE)
