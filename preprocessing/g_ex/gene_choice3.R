...
#sum(
#length(df.gene_data.ex$group[df.gene_data.ex$group==1]) * 0.7
#,
#length(df.gene_data.ex$group[df.gene_data.ex$group==2]) * 0.7
#,
#length(df.gene_data.ex$group[df.gene_data.ex$group==3]) * 0.7
#,
#length(df.gene_data.ex$group[df.gene_data.ex$group==4]) * 0.7
#,
#length(df.gene_data.ex$group[df.gene_data.ex$group==5]) * 0.7
#,
#length(df.gene_data.ex$group[df.gene_data.ex$group==6]) * 0.7
#)

# Write to .csv file
write.csv(df.gene_data.ex,"../../data/gene_ex2/gene_data_ex.csv",row.names=TRUE)

df.gene_data.ex.body.scaled <- scale(df.gene_data.ex[,-c(1:7)])
df.gene_data.ex.scaled <- cbind(df.gene_data.ex[,c(1:7)], df.gene_data.ex.body.scaled)

write.csv(df.gene_data.ex,"../../data/gene_ex2/gene_data_ex_scaled.csv",row.names=TRUE)
