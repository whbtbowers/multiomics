#setwd("/project/home17/whb17/Documents/project3/project_files/preprocessing/ex_12/")
setwd("/home/whb17/Documents/project3/project_files/preprocessing/ex_12/")
...
df.gene.body <- read.csv("../../data/ex_12/gene_data_body.csv", header=TRUE, row.names = 1)
df.prot.body <- read.csv("../../data/ex_12/prot_data_body.csv", header=TRUE, row.names = 1)
df.meta <- read.csv("../../data/ex_12/gp_data_meta.csv", header=TRUE, row.names = 1)
...
write.csv(df.gene.body.log2, "../../data/ex_12/gene_data_body_l2t.csv")
write.csv(df.prot.body.log2, "../../data/ex_12/prot_data_body_l2t.csv")

# Quantile normalise gene and protein data after log2 transformation
t.df.gene.body.log2.qn <- as.data.frame(normalize.quantiles(as.matrix(t.df.gene.body.log2)))
t.df.prot.body.log2.qn <- as.data.frame(normalize.quantiles(as.matrix(t.df.prot.body.log2)))
...
df.prot.body.log2.qn <- as.data.frame(t(t.df.prot.body.log2.qn))
colnames(df.prot.body.log2.qn) <- prot_names
rownames(df.prot.body.log2.qn) <- patient_labels

write.csv(df.gene.body.log2.qn, "../../data/ex_12/gene_data_body_l2t_qn.csv")
write.csv(df.prot.body.log2.qn, "../../data/ex_12/prot_data_body_l2t_qn.csv")
