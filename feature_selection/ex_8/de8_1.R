...
for (i in 1:length(datasets)){
  ...
  for (j in 1:length(comparisons)){
    ...
    # Select by BH adjusted P-values
    ind.dif_ex <- c()
    ...
    # Select patients with log fold change of 0.5 and greater
    
    #lfc.upper <- c()
    
    #for (i in 1:length(tab.res$logFC)){
    #  if (abs(tab.res$logFC[i]) > 0.5){
   #   lfc.upper <- c(lfc.upper, i)
    #   }
    # }
    ...
    #print(sel.tab.res)
    write.csv(sel.tab.res, paste("../../data/", ex_dir, set.abrv, "_", comp.abrv,  "_sig_factors_alpha", alpha, ".csv", sep=""))
