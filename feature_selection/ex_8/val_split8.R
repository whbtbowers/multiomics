...
df.all.meta <- read.csv("../../data/ex_8/gp_data_meta.csv", header=TRUE, row.names = 1)
df.all.meta$group <- as.character(df.all.meta$group)

print("Loading gene data - this will take a while")
df.gene.body <- read.csv("../../data/ex_8/gene_data_body_lt2_qn.csv", header=TRUE, row.names = 1) 

print("Loading protein data")
df.prot.body <- read.csv("../../data/ex_8/prot_data_body_lt2_qn.csv", header=TRUE, row.names = 1)
...
write.csv(df.gene.body.validation, paste("../../data/ex_8/gene_validation_body.csv", sep=""),row.names=TRUE)
write.csv(df.gene.body.test_train, paste("../../data/ex_8/gene_train_body.csv", sep=""),row.names=TRUE)

write.csv(df.prot.body.validation, "../../data/ex_8/prot_validation_body.csv",row.names=TRUE)
write.csv(df.prot.body.test_train, "../../data/ex_8/prot_train_body.csv",row.names=TRUE)

write.csv(df.meta.validation, "../../data/ex_8/gp_validation_meta.csv", row.names=TRUE)
write.csv(df.meta.test_train, "../../data/ex_8/gp_train_meta.csv", row.names=TRUE)
