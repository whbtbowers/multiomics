...
# Complete datasets
#df.gp.all <- read.csv("../../data/ex_9/gp_train_body.csv", header=TRUE, row.names = 1)
...
for (comp in comps){
  ...
  if (comp.origin == "kaforou_2013"){
    ...
    rocs_list <- list(rocs_list, list(drs.roc, origin))
    drs.roc.smooth <- smooth(drs.roc)
    
    png(paste("../../img/", ex_dir, date, comp.origin, "_", comp.abbrv, "_roc.png", sep=""),
