...
# Set up secondary feature selection
...
phase1s = list(
  ...
  ,
  list(sel.gp.tb_nontb, df.meta.tb_nontb, 1, 7, "TB", "LTBI", "HIV-", "tb_nontb_hivneg")
)
...
for (phase1 in phase1s){
  ...
  # Select patients by TB status
  
  ind.sel.pat <- c()
  
  for (i in 1:nrow(df.gp.all)){
    if((df.meta$group[i] == 1) || (df.meta$group[i] == 3)){
      ind.sel.pat <- c(ind.sel.pat, i)
    }
  }
  
  sel.patients.data <- df.gp.all[ind.sel.pat,]
  sel.patients.meta <- df.meta[ind.sel.pat,]
  
  # Construct factors to be considered in analysis
  
  fac.sex <- factor(sel.patients.meta$sex)
  fac.site <- factor(sel.patients.meta$site) # Correct for site? Maybe use as intercept
  fac.tb <- factor(sel.patients.meta$tb.status)
  
  design <- model.matrix(~fac.site + fac.sex + fac.tb)
  
  fit <- lmFit(t(sel.patients.data), design)
  fit <- eBayes(fit, trend=TRUE, robust=TRUE)
  results <- decideTests(fit)
  print(summary(results))
  tab.res <- topTable(fit, coef=4, n=ncol(sel.patients.data))
  print(tab.res)
  ...
  write.csv(sig_rows, paste("../../data/", ex_dir, "feat_sel/", phase1.abbrv, "_phase2_BH_LFC_sig_factors.csv", sep=""))
  
  #Select significant features
  sel.feat.data <- df.gp.all[match(sig_factor, colnames(df.gp.all))]
  ...
  png(paste("../../img/", ex_dir, date, set.abrv, "_", comp.abrv, "_BH_LFC_lasso_phase2_glmnet_coeff.png", sep=""),
      width = 5*300,        # 5 x 300 pixels
      height = 5*300,
      res = 300,            # 300 pixels per inch
      pointsize = 8        # smaller font size
  ...
  png(paste("../../img/", ex_dir, date, phase1.abbrv, "_BH_LFC_lasso_phase2_cv_glmnet_coeff.png", sep=""),
      width = 5*300,        # 5 x 300 pixels
      height = 5*300,
      res = 300,            # 300 pixels per inch
      pointsize = 8        # smaller font size
  ...
  plot(cvfit, main=paste("Cross-validated coefficient plot for", phase1.hivstat, phase1.verb.g1, "vs", phase1.verb.g2, sep=" "))
  dev.off()
  ...
  set.meta.hiv_neg.tb_nontb$group[set.meta.hiv_neg.tb_nontb$group == 5] <- 7
  ...
  for (j in 1:length(comparisons)){
    ...
    sig_factor = rownames(tab.res)[ind.dif_ex]
    sig_rows = tab.res[ind.dif_ex,]
    write.csv(sig_rows, paste("../../data/", ex_dir, "feat_sel/", set.abrv, "_", comp.abrv, "_phase2_BH_LFC_sig_factors.csv", sep=""))
