...
for (i in 1:length(datasets)){
  ...
  for (j in 1:length(comparisons)){
    ...
    # Get factors by sig |LFC|
    
    ind.dif_ex <- c()
    
    
    for (k in 1:length(tab.res$logFC)){
      if (abs(tab.res$logFC[k]) > 0.5){
    ...
    write.csv(sig_rows, paste("../../data/", ex_dir, "feat_sel/",set.abrv, "_", comp.abrv, "_LFC_sig_factors.csv", sep=""))
    ...
    png(paste("../../img/", ex_dir, date, set.abrv, "_", comp.abrv, "_LFC_EMN_glmnet_coeff.png", sep=""),
        width = 5*300,        # 5 x 300 pixels
        height = 5*300,
        res = 300,            # 300 pixels per inch
        pointsize = 8        # smaller font size
    ...
    png(paste("../../img/", ex_dir, date, set.abrv, "_", comp.abrv, "_LFC_EMN_cv_glmnet_coeff.png", sep=""),
        width = 5*300,        # 5 x 300 pixels
        height = 5*300,
        res = 300,            # 300 pixels per inch
        pointsize = 8        # smaller font size
    ...
    write.csv(df.coef.cvfit.lambda1se, paste("../../data/", ex_dir, "feat_sel/",set.abrv, "_", comp.abrv, "_LFC_EMN_sig_factors.csv", sep=""), row.names=1:nrow(df.coef.cvfit.lambda1se))
    ...
    cv1 <- cv.glmnet(data.matrix(sel.comp.data), 
                  data.matrix(as.numeric(comp.meta$group)), 
                  family="gaussian",
                  foldid=foldid,
                  alpha=1
    ...
    cv.5 <- cv.glmnet(data.matrix(sel.comp.data),
                   data.matrix(as.numeric(comp.meta$group)),
                   foldid=foldid,
                   alpha=.5
                   )
    
    cv0 <- cv.glmnet(data.matrix(sel.comp.data),
                  data.matrix(as.numeric(comp.meta$group)),
                  foldid=foldid,
                  alpha=0
                  )
    
    png(paste("../../img/", ex_dir, date, set.abrv, "_", comp.abrv, "_LFC_alpha_comp.png", sep=""),
        width = 5*300,        # 5 x 300 pixels
        height = 5*300,
        res = 300,            # 300 pixels per inch
        pointsize = 8        # smaller font size
