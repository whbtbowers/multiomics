...
sel.p1.gp.tb_od <- read.csv("../../data/ex_9/feat_sel/gp_tb_od_hivneg_BH_LFC_lasso_sig_factors.csv", header=TRUE, row.names = 1)

sel.p1.gp.tb_ltbi <- read.csv("../../data/ex_9/feat_sel/gp_tb_ltbi_hivneg_BH_LFC_lasso_sig_factors.csv", header=TRUE, row.names = 1)

sel.p1.gp.tb_nontb <- read.csv("../../data/ex_9/feat_sel/gp_tb_nontb_hivneg_BH_LFC_lasso_sig_factors.csv", header=TRUE, row.names = 1)
...
sel.phase2.gp.tb_nontb <- read.csv("../../data/ex_9/feat_sel_2/tb_nontb_hivneg_BH_LFC_lasso_phase2_sig_factors.csv", header=TRUE, row.names = 1)
...
comps <- list(
  #list(sel.gp.tb_od, df.meta, "my", 1, 6, "TB", "OD", "HIV-","tb_od_hivneg")
  #,
  #list(sel.gp.tb_ltbi, df.meta, "my", 1, 3, "TB", "LTBI", "HIV-","tb_ltbi_hivneg")
  #,
  #list(sel.gp.tb_ltbi, df.meta, "my", 1, 3, "TB", "LTBI", "HIV-","tb_nontb_hivneg")
  #,
  #list(sel.phase2.gp.tb_od, "", df.meta, "my", 1, 6, "TB", "OD", "HIV-", "tb_od_hivneg_phase2")
  #,
  #list(sel.phase2.gp.tb_ltbi, "", df.meta, "my", 1, 3, "TB", "LTBI", "HIV-", "tb_ltbi_hivneg_phase2")
  #,
  #list(sel.phase2.gp.tb_nontb, "", df.meta.tb_nontb, "my", 1, 7, "TB", "non-TB", "HIV-", "tb_nontb_hivneg")
  #,
...
for (comp in comps){
  comp.sel.features <- comp[[1]]
  comp.meta <- comp[[2]]
  comp.origin <- comp[[3]][1]
  comp.g1 <- comp[[4]][1]
  comp.g2 <- comp[[5]][1]
  comp.verb.g1 <- comp[[6]][1]
  comp.verb.g2 <- comp[[7]][1]
  comp.hivstat <- comp[[8]][1]
  comp.abbrv <- comp[[9]][1]
  ...
  if (comp.origin == "kaforou_2013"){
    ...
    df.upreg <- df.gp.all[,match(upreg.factors, all.probe.ids)]
    df.downreg <- df.gp.all[,match(downreg.factors, all.probe.ids)]
    ...
    for (i in 1:nrow(df.gp.all)){
      drs.i <- sum(df.upreg.my.tb_od[i,]) - sum(df.downreg.my.tb_od[i,])
      drs <- c(drs.my, drs.i)
    }
    ...
    plot(drs.roc,
         col="red",
         lwd=1,
         main=paste("ROC curve for selected factors for", comp.hivstat, "patients," , comp.verb.g1,  "vs", comp.verb.g2, "(Kaforou et al 2013), (DRS)"),
         legacy.axes = TRUE
    )
    ...
    for (i in 1:nrow(comp.sel.features)){
      if (comp.sel.features$reg_dir[i] == "up"){
        upreg.factors <- c(upreg.factors, as.character(comp.sel.factors$features[i]))
        #print(paste("UP:", comp.sel.features$features[i]))
      } else {
        downreg.factors <- c(downreg.factors, as.character(comp.sel.factors$features[i]))
        #print(paste("DOWN:", comp.sel.features$features[i]))
      }
    }
    
    df.upreg <- df.gp.all[match(upreg.factors, colnames(df.gp.all))]
    df.downreg <- df.gp.all[match(downreg.factors, colnames(df.gp.all))]
    ...
    plot(drs.roc,
         col="red",
         lwd=1,
         main=paste("ROC curve for selected factors for", comp.hivstat, "patients," , comp.verb.g1,  "vs", comp.verb.g2, "(DRS)"),
         legacy.axes = TRUE
         )
    ...
    ggplot(data, aes(x=inf.group, y=drs.g1_g2, group=inf.group)) + 
      geom_boxplot() +
      labs(x = "TB status", y="Disease risk score", title =paste("Distribution of DRS values for", comp.hivstat, "patients," , comp.verb.g1,  "vs", comp.verb.g2, "(DRS)")) +
