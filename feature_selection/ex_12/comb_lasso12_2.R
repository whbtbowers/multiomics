...
for (comparison in comparisons){
  ...
  for (k in 1:length(tab.res.gene$logFC)){
    if ((abs(tab.res.gene$logFC[k]) > 0.5) & (tab.res.gene$adj.P.Val[k] < 0.02)){
      ind.dif_ex <- c(ind.dif_ex, k)
    }
  }
  
  sig_P = tab.res.gene$adj.P.Val[ind.dif_ex]
  sig_factor = rownames(tab.res.gene)[ind.dif_ex]
  sig_rows = tab.res.gene[ind.dif_ex,]
  write.csv(sig_rows, paste("../../data/", ex_dir, "feat_sel/gene_", comp.abrv, "_BH_LFC_sig_factors.csv", sep=""))
  #print(comp.verbose)
  #print(nrow(sig_rows))
  #Select significant features
  sel.comp.gene.body <- comp.gene[,match(sig_factor, colnames(comp.gene))]
  
  # Concatenate prot and seleted gene diff ex tables and body data
  #tab.res <- rbind(tab.res.prot, sig_rows)
  sel.comp.data <- cbind(comp.prot, sel.comp.gene.body)
  ...
  beta <- tab.res.gp[match(df.coef.cvfit.lambda1se$features, rownames(tab.res.gp)),]$B
  logFC <- tab.res.gp[match(df.coef.cvfit.lambda1se$features, rownames(tab.res.gp)),]$logFC
  ...
  print(paste("For", comp.verbose, "after DE analysis,", nrow(sig_rows), "gene probes selected. After lasso,", num.sig.factors, "factors selected.", sig.factors.genecount, "genes and", sig.factors.protcount, "proteins.", sep = " "))
