...
df.prot.tt <- read.csv("../../data/ex_7/prot_testtrain_body.csv", header=TRUE, row.names = 1)  # Gene test/train set
...
for (i in 1:length(datasets)){
  ...
  ind.hivneg.tb_ltbi <- c()
  ind.hivneg.tb_od <- c()
  
  for (i in 1:nrow(set.data)){
    if((set.meta$group[i] == 1) || (set.meta$group[i] == 3)){
      ind.hivneg.tb_ltbi <- c(ind.hivneg.tb_ltbi, i)
    }
  }
  
  for (i in 1:nrow(set.data)){
    if((set.meta$group[i] == 1) || (set.meta$group[i] == 5)){
      ind.hivneg.tb_od <- c(ind.hivneg.tb_od, i)
    }
  }
  
  set.data.hiv_neg.tb_ltbi <- set.data[ind.hivneg.tb_ltbi,]
  set.meta.hiv_neg.tb_ltbi <- set.meta[ind.hivneg.tb_ltbi,]
  
  set.data.hiv_neg.tb_od <- set.data[ind.hivneg.tb_od,]
  set.meta.hiv_neg.tb_od <- set.meta[ind.hivneg.tb_od,]
  ...
  for (j in 1:length(comparisons)){
    ...
    mat.design <- model.matrix(~0 + tb_status.factor + sex.factor + site.factor)
    ...
    for (alpha in alphas){
      ...
      #Select significantly differentially expressed components
      
     # coeff.comp.data <- coef(cvfit, s = lamda.0.5se)
      
     # ind.sel.coeff.comp.data <- 
     #   match(names(coeff.comp.data[,1][coeff.comp.data[,1] != 0]), rownames(coeff.comp.data))
      
      #Remove intercept
    #  sel.coeff.comp.data <- coeff.comp.data[ind.sel.coeff.comp.data,]
      
    #  sel.coeff.comp.data <- sel.coeff.comp.data[-1]
      
     # ind.sel.comp.vars <- match(names(sel.coeff.comp.data), colnames(comp.data))
      
     # sel.comp.data <- comp.data[,ind.sel.comp.vars]
      
      # Get GLM coefficients plot for selected components
      
    #  sel.fit.glm <- glmnet(as.matrix(sel.comp.data), 
    #                    comp.meta$group, 
    #                    family="gaussian", 
    #                    alpha=alpha
     # )
      
    #  png(paste("../../img/", ex_dir, date, "de_analysis/sel_comp/", set.abrv, "_", comp.abrv,  "_lamda0.5se_selected_coefficients_alpha", alpha, ".png", sep=""),
    #      width = 5*300,        # 5 x 300 pixels
      #    height = 5*300,
    #      res = 300,            # 300 pixels per inch
      #    pointsize = 8        # smaller font size
     # )
     # plot(sel.fit.glm, label=TRUE)
      #dev.off()
    }
  }
}
