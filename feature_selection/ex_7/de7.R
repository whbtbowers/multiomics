...
for (i in 1:length(datasets)){
  ...
  for (j in 1:length(comparisons)){
    ...
    for (i in 1:length(bh.P_value)){
      if (bh.P_value[i] < 0.05){
        ind.dif_eq <- c(ind.dif_eq, i)
      }
    }
    
    sig_P = bh.P_value[ind.dif_eq]
    sig_factor = rownames(tab.res)[ind.dif_eq]
    ...
    for (alpha in alphas){
     ...
     # png(paste("../../img/", ex_dir, date, set.abrv, "_", comp.abrv,  "_lamda0.5se_selected_coefficients_alpha", alpha, ".png", sep=""),
     #     width = 5*300,        # 5 x 300 pixels
      #    height = 5*300,
      #    res = 300,            # 300 pixels per inch
      #    pointsize = 8        # smaller font size
